    ...
    return function (user, pass) {
        ...
        var succCallBack = function (data, status, response) {
            ...
            if (data.errorCode == '0') {
                ...
                globalModule.alert(data.errorMessage)
            }
        }
...
app.isLR = (function () {
    ...
    function islogin() {
        ...
        if (!user) {
            globalModule.alert('请输入邮箱或手机号码');
            return false
        } else if (!pswd) {
            globalModule.alert('请输入密码');
            return false
        } else if (pswd.length < 6) {
            globalModule.alert('密码不能小于6位');
            return false
        } else {
            app.login(user, pswd)
        }
    ...
    function isregister() {
        ...
        if (!users) {
            ...
            globalModule.alert('请输入正确的手机号码或邮箱');
            return false
        } else if (!pswd1) {
            globalModule.alert('请输入密码');
            return false
            ...
            globalModule.alert('请输入确认密码');
            return false
        } else if (pswd2 !== pswd1) {
            globalModule.alert('密码输入不一致');
            return false
        } else if (!codes) {
            globalModule.alert('请输入验证码');
            return false
        } else {
