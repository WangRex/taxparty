...
f7app.onPageInit('service', function (page) {
                    ...
                    /*$$('#marker1').click(function () {
                        ...
                        var succCallBack = function (data, status, response, address) {
                            ...
                            if (data.data.state == 1) {
                                app.toast("您的资料正在审核当中，请耐心等待。");
                            } else {

                                view.about.router.loadPage('about/application.html');
                            }
                    ...
                    $$('#marker2').click(function () {
                        ...
                        var succCallBack = function (data, status, response, address) {
                            ...
                            if (data.data.state == 1) {
                                ...
                                view.about.router.loadPage('about/service.html');
                            }
    ...
    //初始化工作地点
    var service_region_id_picker = new mui.PopPicker({
        layer: 3
    });

    service_region_id_picker.setData(cityData3);
    $$("#service_region_id").on('click', function (event) {
        console.log(this);
        var self = this;
        service_region_id_picker.show(function (items) {
            var t = (items[0] || {}).text + " " + (items[1] || {}).text + " " + (items[2] || {}).text;
            self.value = t;
            if (items[2].value) {
                self.setAttribute("data_id", items[2].value);
...
//            $$('#agreementBox').css('margin-right', '1px')
        } else {
        ...
        var succCallBack = function (data, status, response) {
            ...
            if (data.data == true) {
                ...
                if (data.data.state == '1') {
                    ...
                    /* var service_tpl = $$('script#service_tpl').html();
                     var tpl = Template7.compile(service_tpl);
                     $$("#service_list").html(tpl(data.data));*/
                    ...
                    var service_region_id = $$("#service_region_name").val(data.data.service_region_name);
                    var service_region_id = $$("#service_region_id").attr("data_id", data.data.service_region_id);
                    var majorList = data.data.majorList;
                    $$('#majorSummary').html(majorList);
                    if (majorList.length == 0) {
                        $$('#majorTitle').append('<div class="item-title" style="line-height: 21px;">专业资格</div>');
                    ...
                    if (taxpropertyList.length == 0) {
                        $$('#taxpropertyTitle').append('<div class="item-title" style="line-height: 21px;">擅长的税务领域</div>');
